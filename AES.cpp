...
int main()
{
	...
	cin >> option1 >> fileName >> outputName >> option >> initialKey;
	cout << "Type 1 for encryption, 0 for decryption" << option1 << endl;
	
	cout << "Type the file to encrypt/decrypt (include the extension)" << fileName<<endl;
	
	cout <<"Type the desired name of the encrypted/decrypted file (include extension)" << outputName << endl;

	char input[32];
	cout << "Type 0 for 32 character hex key, 1 for 16 character key: "<<option<<endl;
	
	if (option == 0)
	{
		cout << "Type a maximum 32 character key to use (note: key must be hexadecimal value, i.e. only characters 0-9 or A-F may be used)" << initialKey << endl;
		
	}
	else
	{
	    cout << "Type a maximum 16 character key to use: " << initialKey<< endl;
